...
const SCREENS: ScreenRow[] = [
  ...
  },
  {
    name: "CR Allocation",
    route: "/cr-allocation",
    description: "Fill in a missing BA and/or ITPM on a CR; assign a Tester to a CR's testing bucket.",
    admin: { level: "full", note: "Assign, reassign, or clear ITPM/BA on any CR missing one, and assign a Tester (unscoped) on any CR needing one." },
    pmo: { level: "full", note: "Same ITPM/BA assignment as Admin. Tester assignment is SPOC-gated like ITPM below, not unscoped." },
    itpm: { level: "scoped", note: "Self-claim only for the ITPM field, and SPOC-gated: sees and can claim the ITPM field only on CRs missing an ITPM whose application is in their own spoc_applications — being unassigned alone isn't enough. Can't touch BA or reassign others. Can also assign a Tester (not self-claim — picking someone else) to any CR needing one, same SPOC gate, to a Tester whose own spoc_applications also covers that application." },
    ba: { level: "scoped", note: "Self-claim only, and SPOC-gated: sees and can claim the BA field only on CRs missing a BA whose application is in their own spoc_applications — being unassigned alone isn't enough. Can't touch ITPM or reassign others. No role in Tester assignment." },
    tester: { level: "none", note: "Explicitly excluded — Testers are assigned here, they don't assign." },
  },
  {
    name: "Data Import",
    route: "/upload",
  ...
  },
  {
    ...
    description: "A Tester's bucket: upload a test case Excel and submit for approval, only for CRs assigned to them from CR Allocation.",
    flag: "Release 2 · testing",
    admin: { level: "full", note: "Sees every CR regardless of tester assignment, and can upload/submit for any of them — the one unscoped exception, for oversight." },
    pmo: { level: "none" },
    itpm: { level: "none" },
    ba: { level: "none" },
    tester: { level: "scoped", note: "Sees, and can upload/submit for, only CRs where crs.tester is them — assigned via CR Allocation. A CR is released back to the unassigned pool once every test case has a final execution outcome." },
  },
  {
  ...
  },
  {
    ...
    tester: { level: "scoped", note: "The only role that can set Execution Status (Tested / Defect Raised) — but only once the CR's test cases are Approved, and only for a CR assigned to them (crs.tester), with no Admin bypass on either check. Cannot approve/send back/comment." },
  },
  {
    name: "CR Planner",
    route: "/cr-planner",
